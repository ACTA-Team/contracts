...
| `register_sponsored` | `register_sponsored(sponsor: Address, did_id: BytesN<16>, initial_record: DidRecord)` |
...
#### 4.4.2 `register_sponsored` - create a DID paid for by a third party

`register_sponsored(sponsor: Address, did_id: BytesN<16>, initial_record: DidRecord)`

Identical to `register` in every respect except who authorizes. The `sponsor`
pays the transaction and resource fees; `initial_record.controller` becomes the
owner of the DID from version 1 without signing anything. The sponsor is never
the controller, so there is no window in which the payer controls the DID.

| Aspect | Behavior |
|---|---|
| **Authorizes** | `sponsor` (`require_auth()`). The controller does **not** sign. |
| **Precondition** | No record exists for `did_id`. Otherwise `DidAlreadyExists` (1). |
| **Precondition** | `sponsor != initial_record.controller`. Otherwise `SponsorIsController` (22). |
| **Validation** | Full `validate_record` per §3.3, identical to `register`. The sponsored path is not a way to submit a record that `register` would reject. |
| **Set by contract** | `version = 1`, `created_ledger = updated_ledger = current ledger`, `deactivated = false`. |
| **Event** | `DidRegisteredSponsored { did_id, sponsor, controller, version: 1 }`. Deliberately a distinct event type rather than an extra field on `DidRegistered`: Soroban events are filtered by topic, so a separate type is what lets a consumer subscribe to sponsored registrations alone. |

**Security considerations.** This operation writes a record naming an address
that never proved control of itself. Two consequences follow, and neither can be
enforced on-chain:

1. **An incorrect `controller` is unrecoverable.** `update`,
   `transfer_controller` and `deactivate` all require the controller's
   authorization. A record written with a mistyped address, an address on the
   wrong network, or an account that does not exist can never be mutated,
   transferred, or retired. It remains resolvable indefinitely, and its TTL is
   extended on every read (§7.6). The only remedy is to register a fresh
   `did_id` and abandon the original.

   Callers MUST therefore validate the controller address off-chain before
   invoking: the account exists on the ledger, it is on the intended network,
   and its signers and thresholds make it usable. Soroban cannot inspect
   account entries, so this cannot be checked by the contract.

2. **Key material MUST be generated by the subject, not the sponsor.** Whoever
   holds the private keys published in `authentication` and `assertionMethod`
   can authenticate and issue credentials as that DID indefinitely, regardless
   of who the controller is - verification per §6 and Annex B.2 never consults
   the controller address. A sponsor that generates the keys retains full
   operational control of the identity it nominally gave away. The subject
   SHOULD generate its own key pair and transmit only the Multikey-encoded
   public key to the sponsor.

The guard on `sponsor == controller` exists because that call is `register` with
extra steps: it would make the sponsor the controller, requiring a subsequent
`transfer_controller` and reintroducing exactly the custody window this
operation is designed to avoid.
...
fn register_sponsored(e: Env, sponsor: Address, did_id: BytesN<16>, initial_record: DidRecord) {
    ...
    // Only the sponsor authorizes. The controller does NOT sign.
...
> **When not to use this.** If the subject already holds a wallet and can sign,
> prefer `register` with a fee-bump transaction: the sponsor wraps the subject's
> signed transaction in a `FeeBumpTransaction` and pays the whole fee, while the
> subject's signature proves control of the controller address. That path costs
> the subject nothing, requires no contract support, and eliminates failure mode
> 1 above. `register_sponsored` is for the case where the subject cannot sign at
> registration time, such as batch enrollment.

---

#### 4.4.3 `update` - rotate keys, change services or metadata
...
> exclusively through `transfer_controller` (§4.4.4), which emits the dedicated
...
#### 4.4.4 `transfer_controller` - hand over on-chain control
...
#### 4.4.5 `deactivate` - permanently retire a DID
...
**Example.** Deactivating the DID from §4.4.3 (version 2) yields the tombstone:
...
#### 4.4.6 Per-operation error matrix

| Error (code) | `register` | `register_sponsored` | `update` | `transfer_controller` | `deactivate` |
|---|:--:|:--:|:--:|:--:|:--:|
| `DidAlreadyExists` (1) | ✔ | ✔ | | | |
| `DidNotFound` (2) | | | ✔ | ✔ | ✔ |
| `VersionMismatch` (3) | | | ✔ | ✔ | ✔ |
...
`register_sponsored` raises exactly the same errors as `register`, plus
`SponsorIsController`. It never raises the version-guarded errors, since like
`register` it takes no `expected_version`.
...
| `DidRegisteredSponsored` | `did_id`, `sponsor`, `controller`, `version` |
...
3. The on-chain `DidRecord.controller` reflects the new controller address immediately after the transaction is confirmed (semantics in §4.4.4).

#### 4.6.4 Deactivation Flow

1. The client reads the current `DidRecord` and notes `version`.
2. The current controller signs `deactivate(did_id, expected_version)`.
3. After confirmation the DID is a tombstone and irreversible; a new identity requires a fresh `didId` and a new `register` call (semantics in §4.4.5).
...
| `SponsorIsController` (22) | `register_sponsored` was called with `sponsor == initial_record.controller`. | **Fix input:** use `register` instead. The sponsored path exists only for the case where the two addresses differ. |
...
| `POST` | `/v1/dids/stellar/{did}/update` | Prepare an `update` (§4.4.3); requires `expectedVersion`. |
| `POST` | `/v1/dids/stellar/{did}/transfer` | Prepare a `transfer_controller` (§4.4.4); requires `expectedVersion` and `newController`. |
| `POST` | `/v1/dids/stellar/{did}/deactivate` | Prepare a `deactivate` (§4.4.5); irreversible once submitted. |
...
(§4.4.3). The contract bumps to `version = 2`; the old key vanishes from every
...
(§4.4.4) → `version = 3`. Keys, services, and the resolved DID Document are
...
`deactivate(did_id, expected_version = 3)` (§4.4.5) → `version = 4`,
