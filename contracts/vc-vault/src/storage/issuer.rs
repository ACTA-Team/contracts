...
pub fn read_issuer_count(e: &Env) -> u32 {
    e.storage()
        .persistent()
        .get(&VcVaultDataKey::VaultIssuerCount)
        .unwrap_or(0)
}

pub fn write_issuer_count(e: &Env, count: u32) {
    let key = VcVaultDataKey::VaultIssuerCount;
...
pub fn read_issuer_at(e: &Env, position: u32) -> Option<Address> {
    e.storage()
        .persistent()
        .get(&VcVaultDataKey::VaultIssuerIndex(position))
}

pub fn read_issuer_at_extend(e: &Env, position: u32) -> Option<Address> {
    let key = VcVaultDataKey::VaultIssuerIndex(position);
...
pub fn write_issuer_at(e: &Env, position: u32, issuer: &Address) {
    let key = VcVaultDataKey::VaultIssuerIndex(position);
...
pub fn remove_issuer_at(e: &Env, position: u32) {
    e.storage()
        .persistent()
        .remove(&VcVaultDataKey::VaultIssuerIndex(position));
}

pub fn read_issuer_position(e: &Env, issuer: &Address) -> Option<u32> {
    e.storage()
        .persistent()
        .get(&VcVaultDataKey::VaultIssuerPosition(issuer.clone()))
}

pub fn write_issuer_position(e: &Env, issuer: &Address, position: u32) {
    let key = VcVaultDataKey::VaultIssuerPosition(issuer.clone());
...
pub fn remove_issuer_position(e: &Env, issuer: &Address) {
    e.storage()
        .persistent()
        .remove(&VcVaultDataKey::VaultIssuerPosition(issuer.clone()));
}

pub fn issuer_index_contains(e: &Env, issuer: &Address) -> bool {
    e.storage()
        .persistent()
        .has(&VcVaultDataKey::VaultIssuerPosition(issuer.clone()))
}

pub fn append_issuer_to_index(e: &Env, issuer: &Address) {
    let count = read_issuer_count(e);
    if count >= MAX_ISSUERS_LIST {
        panic_with_error!(e, ContractError::IssuerListTooLong);
    }
    write_issuer_at(e, count, issuer);
    write_issuer_position(e, issuer, count);
    write_issuer_count(e, count + 1);
}

pub fn remove_issuer_from_index(e: &Env, issuer: &Address) {
    let position = match read_issuer_position(e, issuer) {
        Some(p) => p,
        None => return,
    };
    let count = read_issuer_count(e);
    ...
    let last = count - 1;
    if position != last {
        let last_addr = read_issuer_at(e, last).unwrap();
        write_issuer_at(e, position, &last_addr);
        write_issuer_position(e, &last_addr, position);
    }
    remove_issuer_at(e, last);
    remove_issuer_position(e, issuer);
    write_issuer_count(e, last);
}

pub fn clear_issuer_index(e: &Env) {
    let count = read_issuer_count(e);
    for i in 0..count {
        if let Some(addr) = read_issuer_at(e, i) {
            remove_issuer_position(e, &addr);
        }
        remove_issuer_at(e, i);
    }
    write_issuer_count(e, 0);
}
